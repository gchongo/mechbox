...
const BELT = {
  zh: {
    ...
    modesSubtitle: '开口/V 带；Euler 紧松边张力；完整起校核带速',
    modes: stdCalcModes('zh', {
      simpleModel: '带长、传动比、带速；wrapAngle 默认 180°；Euler $F_1/F_2$',
      completeModel: '几何包角 calcWrapAngle + **带速校核** speedPass + 根数估算',
      proModel: '完整 + 使用系数 serviceFactor + 挠曲应力 flexPass + 寿命估算',
      simplePass: '无正式 pass（仅输出张力与几何）',
      completePass: '**speedPass**：$v \\le v_{max}$（默认 30 m/s）',
      proPass: 'speedPass $\\land$ flexPass（$F_1/A_{sect}\\le[\\sigma_t]$）',
      simpleCaveat: '包角可手填；不看带速上限',
      completeCaveat: 'powerPerBelt 默认取当前功率 — 根数常为 1',
      proCaveat: '寿命为 $10^4\\times(\\sigma/F_1)^3$ 量级估算',
    }),
    formulasTitle: '主要公式',
    formulas: [
      { name: '带长', latex: 'L = 2C + \\frac{\\pi(D_1+D_2)}{2} + \\frac{(D_2-D_1)^2}{4C}', note: '开口传动' },
      { name: '带速', latex: 'v = \\frac{\\pi D_1 n}{60000}', note: 'm/s；$D$ mm，$n$ rpm' },
      { name: '有效圆周力', latex: 'F = \\frac{P}{v\\eta}', note: '设计功率含 serviceFactor（专业）' },
      { name: 'Euler 张力', latex: 'F_1 = F_2 e^{\\mu\\theta}, \\quad F = F_1 - F_2', note: '$\\theta$ 弧度；wrapAngle 度输入' },
      { name: '根数', latex: 'z = \\lceil P K_s / P_{belt} \\rceil', note: 'complete 中 $K_s=1$；pro 用 serviceFactor' },
    ],
    passTitle: '判定依据',
    passChecks: [
      { check: 'speedPass', rule: '完整/专业：beltSpeed $\\le$ maxBeltSpeed（默认 30 m/s）' },
      { check: 'flexPass（专业）', rule: 'flexStress $=F_1/(beltSection)\\le allowTension$（默认 600）' },
      { check: '综合 pass', rule: 'complete：speedPass；pro：speedPass $\\land$ flexPass' },
      { check: '包角', rule: 'complete 由 $D_1,D_2,C$ 算 wrapAngle，限制 120°–210°' },
      { check: '简化模式', rule: '无 pass 字段 — 仅估算张力' },
    ],
    guideTitle: '使用指南',
    guideIntro: '输入两轮直径、中心距、功率与转速；完整模式自动算包角并校核带速。',
    guideSections: [
      {
        title: '参数来源',
        bullets: [
          'power 或由 torque×rpm 换算',
          'friction 默认 0.3（V 带橡胶）',
          'efficiency 默认 0.95',
        ],
      },
      {
        title: '专业模式',
        bullets: [
          'serviceFactor 放大设计功率（默认 1.2）',
          'beltSection 为带宽×厚近似截面积（mm²）',
          'estimatedLifeHours 供方案比较',
        ],
      },
    ],
    examplesTitle: '算例要点',
    examples: [
      { step: '几何', detail: '$D_1=120,D_2=240,C=400$ → 带长、包角、$i=2$' },
      { step: '张力', detail: '$P=5$ kW，$n=1450$ → $F$、$F_1$、$F_2$' },
      { step: '带速', detail: '$D_1=120,n=3000$ → $v\\approx18.8$ m/s；超 30 则 fail' },
    ],
    faqTitle: '常见问题',
    faq: [
      { q: '简化模式能 pass 吗？', a: '不算 pass；仅输出几何与张力量级。' },
      { q: '包角 180° 从哪来？', a: '简化默认；完整由中心距与直径几何计算。' },
      { q: '交叉传动？', a: '当前带长公式为开口平行传动；交叉需另式。' },
    ],
    howToPassTitle: '如何让判定通过',
    howToPass: [
      { goal: '带速过高', steps: ['减小小轮直径或转速', '提高 maxBeltSpeed（需带型允许）', '改用多根带分担'] },
      { goal: '挠曲应力（专业）', steps: ['增大 beltSection（更宽带型）', '降低 F₁（更大包角/摩擦）', '提高 allowTension'] },
      { goal: '打滑风险', steps: ['增大包角（增大 C 或直径差）', '提高摩擦系数 μ', '增加预紧/根数'] },
    ],
    standardsTitle: '相关标准与资料',
    standardsIntro: '教材 Euler 公式；选型请对照厂家 V 带/同步带样本。',
    standards: [
    ...
    limits: [
      ...
      '开口平行布置；未含张紧轮自动调节。',
      '寿命估算为指数经验式，非厂家 L10 曲线。',
    ],
  },
  en: {
    modesTitle: 'Belt drive — calculation modes',
    modesSubtitle: 'Open/V-belt; Euler tight/slack tension; Full adds speed check',
    modes: stdCalcModes('en', {
      simpleModel: 'Length, ratio, speed; default wrapAngle 180°; Euler F₁/F₂',
      completeModel: 'Geometric wrap angle + **speedPass** + belt count estimate',
      proModel: 'Full + serviceFactor + flexPass + life estimate',
      simplePass: 'No formal pass (geometry and tension only)',
      completePass: '**speedPass**: $v \\le v_{max}$ (default 30 m/s)',
      proPass: 'speedPass $\\land$ flexPass',
      simpleCaveat: 'Manual wrap angle; no speed cap',
      completeCaveat: 'powerPerBelt defaults to current power—count often 1',
      proCaveat: 'Life is rough $10^4(\\sigma/F_1)^3$ scale',
    }),
    formulasTitle: 'Key formulas',
    formulas: [
      { name: 'Belt length', latex: 'L = 2C + \\pi(D_1+D_2)/2 + (D_2-D_1)^2/(4C)', note: 'Open drive' },
      { name: 'Belt speed', latex: 'v = \\pi D_1 n / 60000', note: 'm/s' },
      { name: 'Effective force', latex: 'F = P/(v\\eta)', note: 'Design power includes serviceFactor in Pro' },
      { name: 'Euler tension', latex: 'F_1 = F_2 e^{\\mu\\theta}', note: 'θ in radians' },
      { name: 'Belt count', latex: 'z = \\lceil P K_s / P_{belt} \\rceil', note: 'Pro uses serviceFactor' },
    ],
    passTitle: 'Pass criteria',
    passChecks: [
      { check: 'speedPass', rule: 'Full/Pro: beltSpeed $\\le$ maxBeltSpeed (default 30 m/s)' },
      { check: 'flexPass (Pro)', rule: '$F_1/beltSection \\le allowTension$' },
      { check: 'Overall pass', rule: 'Full: speedPass; Pro: speedPass $\\land$ flexPass' },
      { check: 'Wrap angle', rule: 'Full: from geometry, clamped 120°–210°' },
      { check: 'Simple', rule: 'No pass—tension estimate only' },
    ],
    guideTitle: 'How to use',
    guideIntro: 'Enter pulley diameters, center distance, power and speed; Full mode checks belt speed.',
    guideSections: [
      { title: 'Inputs', bullets: ['Power from torque×rpm if omitted', 'friction default 0.3', 'efficiency default 0.95'] },
      { title: 'Professional', bullets: ['serviceFactor on design power', 'beltSection ≈ cross area mm²', 'estimatedLifeHours for comparison'] },
    ],
    examplesTitle: 'Example notes',
    examples: [
      { step: 'Geometry', detail: 'D₁,D₂,C → length, wrap angle, ratio' },
      { step: 'Tension', detail: '5 kW, 1450 rpm → F, F₁, F₂' },
      { step: 'Speed', detail: 'Check v vs 30 m/s default limit' },
    ],
    faqTitle: 'FAQ',
    faq: [
      { q: 'Simple mode pass?', a: 'No pass field—estimate only.' },
      { q: '180° wrap?', a: 'Simple default; Full computes from geometry.' },
      { q: 'Crossed belts?', a: 'Open parallel formula only.' },
    ],
    howToPassTitle: 'How to pass',
    howToPass: [
      { goal: 'Speed fail', steps: ['Smaller pulley or rpm', 'Raise limit if belt allows', 'More belts'] },
      { goal: 'Flex stress (Pro)', steps: ['Larger belt section', 'Lower F₁', 'Higher allowTension'] },
      { goal: 'Slip risk', steps: ['Larger wrap angle', 'Higher μ', 'More belts / pretension'] },
    ],
    standardsTitle: 'References',
    standardsIntro: 'Euler textbook model; use manufacturer catalogs for selection.',
    standards: ['GB/T 11544 / ISO 4184 — V-belts (reference)', 'Handbook — belt tension and wrap'],
    limitsTitle: 'Limitations',
    limits: [
      'Steady drive; limited centrifugal modeling.',
      'Open parallel layout; no automatic tensioner model.',
      'Life estimate is empirical—not manufacturer L10.',
    ],
  },
}
