export const GLOSSARY_TERMS = [
  ...
  },
  {
    ...
    category: '过程能力',
    definition:
      'Cpk 衡量生产过程「能否稳定落在规格范围内」，同时考虑过程中心是否偏移。USL/LSL 是规格上下限，$\\mu$ 是过程均值，$\\sigma$ 是标准差。Cpk 取「距上规格」和「距下规格」中较小的一侧除以 $3\\sigma$。Cpk $\\geq 1.33$ 通常认为过程能力良好；小于 1 说明波动太大或中心偏了，不合格风险高。结果页会给出当前 Cpk 与对照表供参考。',
    tags: ['SPC', '质量'],
  },
  {
    id: 'sigma-level',
    ...
    category: '过程能力',
    definition:
      '西格玛水平描述过程变异相对规格带宽的宽窄，数值越大表示过程越稳定、越不容易出不合格品。在正态近似下，$3\\sigma$ 约对应 99.73% 合格率，$6\\sigma$ 则接近零缺陷。工具中用目标公差与 RSS 合成 $\\sigma$ 估算 $\\sigma$ 水平，并进一步换算合格率与 DPPM（百万分之不合格数）。',
    tags: ['6σ', '合格率'],
  },
  {
    id: 'monte-carlo',
  ...
  },
  {
    ...
    category: '统计方法',
    definition:
      '分布系数 K 用于把公差 T 换算成标准差：$\\sigma = T/K$。不同分布形状 K 不同——正态常用 K=6（$\\pm 3\\sigma$ 覆盖规格），均匀分布 K≈3.46，三角分布 K≈4.24。$6\\sigma$ RSS 法会用各环的 K 把公差还原为 $\\sigma$ 后再做统计叠加，比简单 RSS 更严谨。',
    tags: ['转换', '分布'],
  },
  {
    id: 'agma-geometry-i',
