    ...
    <section class="mb-8">
      ...
      <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <div
          ...
          class="card-panel"
        >
          <h3 class="mb-3 font-medium text-primary">{{ group.label }}</h3>
          <div class="space-y-2">
            <button
              v-for="type in group.types"
              :key="type.id"
              class="flex w-full items-center justify-between rounded-md px-3 py-2 text-left text-sm transition-colors hover:bg-primary/5"
      ...
      <div v-if="history.length" class="space-y-3">
        ...
        >
          ...
          <div class="flex gap-2">
            ...
            <el-button size="small" type="danger" plain @click="removeHistory(item.id)">
              删除
            </el-button>
          </div>
        </div>
