<template>
  <div class="mech-diagram">
    ...
    <svg class="mech-diagram__svg" viewBox="0 0 480 260" role="img" :aria-label="dt('aria')">
      ...
      <SvgMathText :x="cxSec" :y="cySec + rOuter + 34" :text="dl('d', diameter)" anchor="middle" class-name="txt-primary" color="#409eff" :width="120" :font-size="13" />
      ...
      <SvgMathText v-if="mode === 'torsion' && length > 0" :x="shaftX + shaftLen / 2" :y="shaftY + shaftH + 38" :text="dl('L', length)" anchor="middle" class-name="txt" color="#334155" :width="120" :font-size="12" />
    </svg>
  </div>
</template>
...
const { dt, dm, dl } = useDiagramI18n('shaft')
